...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var map:FlxSprite = new FlxSprite().loadGraphic(Paths.image('storymode/map'));
		map.updateHitbox();
		map.width = FlxG.width;
		map.height = FlxG.height;
		map.screenCenter();
		map.y -= 10000;
		map.antialiasing = ClientPrefs.globalAntialiasing;
		add(map);

		map1 = new FlxSprite().loadGraphic(Paths.image('storymode/map_0'));
		map1.updateHitbox();
		map1.width = FlxG.width;
		map1.height = FlxG.height;
		map1.screenCenter();
		map1.alpha = 0;
		map1.antialiasing = ClientPrefs.globalAntialiasing;
		add(map1);

		mapNothing = new FlxSprite().loadGraphic(Paths.image('storymode/noMap'));
		mapNothing.updateHitbox();
		mapNothing.width = FlxG.width;
		mapNothing.height = FlxG.height;
		mapNothing.screenCenter();
		mapNothing.alpha = 0;
		mapNothing.antialiasing = ClientPrefs.globalAntialiasing;
		add(mapNothing);

		var darkThing:FlxSprite = new FlxSprite().loadGraphic(Paths.image('mainmenu/frontBG'));
		darkThing.updateHitbox();
		darkThing.width = FlxG.width;
		darkThing.height = FlxG.height;
		darkThing.screenCenter();
		darkThing.alpha = 0.9;
		darkThing.antialiasing = ClientPrefs.globalAntialiasing;
		add(darkThing);
		...
		// Funny Moves
		FlxTween.tween(map, {y: map.y + 10000}, 1, {
			ease: FlxEase.expoInOut,
			onComplete: function(twn:FlxTween)
			{
				FlxTween.tween(map, {alpha: 0}, 1, {ease: FlxEase.quadInOut});
				FlxTween.tween(map1, {alpha: 1}, 1, {
	...
	function selectWeek()
	{
		if (!weekIsLocked(curWeek))
		{
			...
			FlxTween.tween(map1, {alpha: 0}, 0.5, {ease: FlxEase.quadInOut});
			FlxTween.tween(mapNothing, {alpha: 1}, 0.5, {ease: FlxEase.quadInOut});
